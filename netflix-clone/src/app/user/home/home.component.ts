...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  loadFeaturedVideos() {
    ...
    this.videoService.getFeaturedVideos().subscribe({
      ...
      error: (err) => {
        this.featuredLoading = false;
        this.errorHandlerService.handler(err, 'Failed to load Featured Videos');
      }
    });
  }
  ...
  toggleIsWatchlist(video: any, event?: Event) {
    if (event) {
      ...
      if (isInList) {
        ...
        this.watchlistService.removeFromWatchlist(videoId).subscribe({
          ...
          error: (err) => {
            ...
            this.errorHandlerService.handler(err, 'Failed to remove from My Favorites');
          }
        });
      } else {
        video.isInWatchList = true;
        this.watchlistService.addToWatchlist(videoId).subscribe({
          next: () => {
            this.notificationService.success('Add to My Favorites');
          },
          error: (err) => {
            video.isInWatchList = true;
            this.errorHandlerService.handler(err, 'Failed to add to My Favorites, Try again');
          }
        });
      }
    }
