...
import { RATINGS, VIDEO_CATEGORIES } from '../../../shared/contants/app.constants';
import { AbstractControl, FormBuilder, RequiredValidator, ValidationErrors, Validators } from '@angular/forms';
...
export class ManageVideoComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ManageVideoComponent>,
  ...
  onVideoPicked(ev: Event) {
    ...
    if (!hasValidMimeType && !hasValidExtension) {
      this.notificationService.error('Please select a valid video file (MP4, MKV, etc)');
      return;
    }
    ...
    this.mediaService.uploadFile(file).subscribe({
      ...
      error: (err) => {
        this.notificationService.error('Failed to upload video. Please try again.');
        this.uploadProgress = 0;
        if (this.posterPreviewUrl === localBlobUrl) {
  ...
  onPosterPicked(ev: Event) {
    ...
    this.mediaService.uploadFile(file).subscribe({
      ...
      error: (err) => {
        this.notificationService.error('Failed to upload poster. Please try again.');
        this.posterProgress = 0;
  ...
  onSave() {
    ...
    op$.subscribe({
      ...
      error: (err) => {
        this.isSaving = false;
        this.errorHandleService.handler(err, 'Failed to save video. Please try again.');
      }
    })
  }
